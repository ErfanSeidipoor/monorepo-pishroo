...
import { DASHBOARD_TRANSPORTER_AGENT_NEW_TRANSPORTER_AGENT_ROUTE } from "@admin/constants";

import useData from "./useDate";

export const TransporterAgentListPage: FC = () => {
  const {
    rows,
    loading,
    navigate,
  ...
  const columns: ITableColumn<typeof rows[0]>[] = [
    ...
    },
    {
      ...
      cell: (item) => (
        <p>
          {item.transporter.city.name} <br />
          {item.transporter.city.province.name}
        </p>
      ),
      label: TEXTS.CITY,
    },
      ...
      >
        ...
        >
          <Typography variant="h4">
            {TEXTS.PAGE_TRANSPORTER_AGENT__TRANSPORTER_AGENT}
          </Typography>
          {renderFilterSidebar()}
        </Stack>
        <Button
          variant="outlined"
          label={TEXTS.PAGE_TRANSPORTER_AGENT__NEW_TRANSPORTER_AGENT}
          color="secondary"
          onClick={() =>
            navigate(DASHBOARD_TRANSPORTER_AGENT_NEW_TRANSPORTER_AGENT_ROUTE)
          }
          startIcon={<AddIcon />}
        ></Button>
      </Stack>
    ...
    return (
      ...
      >
        ...
        <DialogActions>
          ...
          />
          <Button
            onClick={onUpdateTransporterAgentActivation}
            label={TEXTS.CONFIRM}
          />
        </DialogActions>
      </Dialog>
...
export default TransporterAgentListPage;
