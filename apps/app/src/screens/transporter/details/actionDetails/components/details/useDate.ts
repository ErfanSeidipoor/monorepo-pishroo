...
import { UpdateTransporterActionAdminInputs } from "libs/dto/src/admin";
import TEXTS from "libs/texts/src";

import { MUTATION_UPDATE_TRANSPORTER_ACTION_ADMIN } from "./gql";

const useData = () => {
  const { getTransporterActionLoading, transporterActionId, transporterAction } =
  ...
  const [updateTransporterActionAdmin, { loading: updateTransporterActionLoading }] =
    useMutation<
      UpdateTransporterActionAdminInputsMutation,
      UpdateTransporterActionAdminInputsMutationVariables
